...
function checkParams() {
  ...
  //проверка фио
  if (!name.value) {
    document.querySelector("div.error_fio").textContent =
      "Пожалуйста, заполните поле ФИО"
    document.querySelector("div.error_fio").style.visibility = "visible"
    name.focus()
    return false
  } else {
    document.querySelector("div.error_fio").style.visibility = "Hidden"
    ...
    //проверка почты
    if (!email.value) {
      document.querySelector("div.error_email").textContent =
        "Пожалуйста, заполните поле email"
      document.querySelector("div.error_email").style.visibility = "visible"
      email.focus()
      return false
    } else {
      document.querySelector("div.error_email").style.visibility = "Hidden"
      if (!re.test(em)) {
        document.querySelector("div.error_email").textContent =
      ...
      //проверка телефона
      if (!phone.value) {
        document.querySelector("div.error_tele").textContent =
        ...
        if (!regex.test(tel)) {
          document.querySelector("div.error_tele").textContent =
            "Поле телефон может начинаться только с +7 или +3"
          document.querySelector("div.error_tele").style.visibility = "visible"
          phone.focus()
          return false
        } else {
