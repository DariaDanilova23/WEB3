...
let yearSelector = document.querySelector("select.year").firstChild
  .nextElementSibling
...
function makeTableDate(choosedMonth) {
  ...
  if (1 < choosedMonth.getDay())
    choosedMonth = new Date(
      choosedMonth.getFullYear(),
      choosedMonth.getMonth(),
      1 - choosedMonth.getDay() + 1
    )
